...
async function requestPokemons(req,res,next){
    try{
        const API_Answer = await axios.get('https://pokeapi.co/api/v2/pokemon?limit=40')
        const API_Pokemons = [];
        for await (const element of API_Answer.data.results) {
            ...
            const pokemon = {
                ...
                types: response.types.map(el => ({ name: el.type.name, image: `https://typedex.app/types/${el.type.name}.png` }))
            }
...
async function getPokemonByName(req,res,next) {
    ...
    if(pkm === null){
        try{
            const answer = await axios.get(`https://pokeapi.co/api/v2/pokemon/${name}`)
            const response = answer.data;
            const pokemon = {
                id: response.id,
                name: response.name,
